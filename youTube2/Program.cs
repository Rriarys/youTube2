...
internal class Program
{
    static void Main()
    {
        ...
        // same, but with repeat
        while (true)

        {
            Console.Write("Enter height of rombus (3 and more): "); int user_rows = Convert.ToInt32(Console.ReadLine());

            Console.Write("How much times repeat: "); int repeat = Convert.ToInt32(Console.ReadLine());

            Console.WriteLine();

            int repeatsCounter = 0, repeatsInRow = 0; //счетчики общий и для строк

            while (repeatsCounter < repeat)
            {
                int half_rows = user_rows / 2; //разбиваем ромб пополам, если 5 => будет по 2 на каждую сторону
                ...
                for (int i = 1; i <= half_rows + 1; i++) //верх + 1 (центр будет рассчитан в верхней), начинаем не с 0, 
                                                         //чтобы не получить пустую строку в первом ряду
                {
                    for (int sp = 0; sp < repeatsInRow; sp++) // sp - spaces
                    {
                        Console.Write("   "); //3 пробелов
                    }
                ...
                if (repeatsInRow == 5) // 5 в каждую строку
                {
                    Console.WriteLine();
                    repeatsInRow = 0;
                }
